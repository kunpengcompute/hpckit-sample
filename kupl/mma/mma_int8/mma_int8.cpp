/*
    KUPL int8精度MMA 矩阵编程 Demo
    编译命令: clang++ mma_int8.cpp -o mma_int8 -O3 -lkupl
    运行命令: taskset -c 0 ./mma_int8
*/
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
void init_matrix_data(int8_t *data_a, int8_t *data_b, int32_t *data_c);
void prepack_matrix_a(int8_t *data_a, int8_t *pack_data_a, int m, int k);
void prepack_matrix_b(int8_t *data_b, int8_t *pack_data_b, int k, int n);
void print_matrix(int32_t *data, int m, int n);

int main()
{
    // 用户原始矩阵数据 Buffer 创建
    int8_t *data_a = (int8_t*)malloc(sizeof(int8_t) * MATRIX_M * MATRIX_K); // row-major
    int8_t *data_b = (int8_t*)malloc(sizeof(int8_t) * MATRIX_K * MATRIX_N); // col-major
    int32_t *data_c = (int32_t*)malloc(sizeof(int32_t) * MATRIX_M * MATRIX_N); // rou-major
    init_matrix_data(data_a, data_b, data_c);
    int8_t *pack_data_a = (int8_t*)malloc(sizeof(int8_t) * MATRIX_M * MATRIX_K);
    int8_t *pack_data_b = (int8_t*)malloc(sizeof(int8_t) * MATRIX_K * MATRIX_N);
    ...
    auto tiled_store = make_tiled_store(Ops<STORE_16x64_INT32>{}, atom_store_shape);
...
void init_matrix_data(int8_t *data_a, int8_t *data_b, int32_t *data_c)
{
    for (int i = 0; i < MATRIX_M; i++) {        // row-major
        for (int j = 0; j < MATRIX_K; j++) {
            if (i == j) {
                data_a[i * MATRIX_K + j] = (int8_t)1;
            } else {
                data_a[i * MATRIX_K + j] = (int8_t)0;
            }
        }
    }
    ...
    for (int i = 0; i < MATRIX_N; i++) {        // col-major
        for (int j = 0; j < MATRIX_K; j++) {
            data_b[i * MATRIX_K + j] = (int8_t)((j * MATRIX_N + i) % 100);
        }
    }

    for (int i = 0; i < MATRIX_M; i++) {           // row-major
        for (int j = 0; j < MATRIX_N; j++) {
            data_c[i * MATRIX_N + j] = (int32_t)0;
        }
    }
}
...
void prepack_matrix_a(int8_t *data_a, int8_t *pack_data_a, int m, int k)
{
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < k; j++) {
            pack_data_a[j / 4 * m * 4 + i * 4 + j % 4] = data_a[i * k + j];
        }
    }
}
...
void prepack_matrix_b(int8_t *data_b, int8_t *pack_data_b, int k, int n)
{
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < k; j++) {
            pack_data_b[j / 4 * n * 4 + i * 4 + j % 4] = data_b[i * k + j];
        }
    }
}
